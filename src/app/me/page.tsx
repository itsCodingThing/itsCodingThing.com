...
export default function MePage() {
	...
	return (
		<div
			className={`min-h-screen transition-colors duration-300 ${dark ? "bg-[#0a0a0a] text-slate-300" : "bg-[#fafafa] text-slate-800"}`}
		>
			<button
			...
			<div
				className={`max-w-3xl mx-auto px-8 py-20 ${dark ? "border-slate-800" : "border-slate-200"}`}
			>
				<header className="mb-32">
					<p
						className={`text-sm mb-4 ${dark ? "text-slate-500" : "text-slate-400"}`}
					>
						Hello, I am
					</p>
					<h1
						className={`text-5xl font-light mb-6 tracking-tight ${dark ? "text-white" : ""}`}
					>
						Bhanu Pratap Singh
					</h1>
					<p
						className={`text-xl font-light leading-relaxed max-w-lg ${dark ? "text-slate-400" : "text-slate-500"}`}
					>
					...
					<div className="flex gap-6 mt-8">
						<a
							href="#work"
							className={`text-sm font-medium hover:text-blue-500 transition-colors ${dark ? "text-slate-300" : ""}`}
						>
							View Work
						</a>
						<a
				...
				<section id="about" className="mb-32">
					<h2
						className={`text-sm font-medium uppercase tracking-widest mb-8 ${dark ? "text-slate-500" : "text-slate-400"}`}
					>
						About
					</h2>
					<div
						className={`space-y-6 leading-relaxed ${dark ? "text-slate-400" : "text-slate-600"}`}
					>
						<p>
				...
				<section id="skills" className="mb-32">
					...
					<div className="grid grid-cols-2 gap-6">
						{[
							...
							<div key={group.label}>
								<p
									className={`text-xs font-medium uppercase tracking-wide mb-3 ${dark ? "text-slate-500" : "text-slate-400"}`}
								>
									{group.label}
								</p>
					...
					<div className="space-y-12">
						{projects.map((project) => (
							<div key={project.title} className="group">
								<h3
									className={`text-lg font-medium mb-2 transition-colors ${dark ? "text-slate-200 group-hover:text-white" : "text-slate-800 group-hover:text-slate-600"}`}
								>
									{project.title}
								</h3>
								<p
									className={`mb-4 leading-relaxed max-w-lg ${dark ? "text-slate-400" : "text-slate-500"}`}
								>
									{project.description}
								</p>
				...
				<section id="contact" className="mb-20">
					...
					<p
						className={`mb-8 leading-relaxed max-w-lg ${dark ? "text-slate-400" : "text-slate-500"}`}
					>
					...
					<div className="flex gap-4">
						...
						<div className="flex gap-2">
							{[
								...
								{ icon: FiTwitter, href: "https://twitter.com/itscodingthing" },
							].map((social) => (
								<a
									key={social.href}
									href={social.href}
				...
				<footer
					className={`pt-8 ${dark ? "border-slate-800" : "border-slate-200"} border-t`}
				>
					<p
